...
| File **upload** (PC→phone) | HTTP `/transport/upload_files_info` + `/upload/upload_files` (TLS `:10380`) — **not** native VDFS | 🟡 `connect --send`: announce step verified live; byte-write 500s server-side (needs a capture) |
